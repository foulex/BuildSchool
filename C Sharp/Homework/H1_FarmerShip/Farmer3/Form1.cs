...
namespace Farmer3
{
    public partial class Form1 : Form
    {
        ...
        {
            if (LeftSide.SelectedItem != null && (string)LeftSide.SelectedItem != "農夫" && FarmerAtLeft==true)
            {
                LList.Remove((string)LeftSide.SelectedItem); RList.Add((string)LeftSide.SelectedItem);
            }
            else if (RightSide.SelectedItem != null && (string)RightSide.SelectedItem != "農夫" && FarmerAtLeft == false)
            {
            ...
            if ((FarmerAtLeft == true && WhereSheep == 1) || (FarmerAtLeft == false && WhereSheep == 0)) {/*農夫跟綿羊在一起，一切安全不會出事。*/}
